...
//  self = allTypes;
  ...
  $scope.lists = [];
    ...
    for(var i=0;i<$scope.lists.length;i++)
    {
      ...
      $scope.listItems.push({"item" : [localStorage.getItem($scope.lists[i])],
       "key" : [$scope.lists[i]], 'type' : [searchTaskType($scope.lists[i],$scope.db.taskType)]});
...
//controller for the pages/pending.html
